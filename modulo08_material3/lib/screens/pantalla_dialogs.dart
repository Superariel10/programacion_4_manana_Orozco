...
class PantallaDialogs extends StatelessWidget {
  ...
  Future<void> _mostrarTest(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        icon:    const Icon(Icons.warning_amber, color: Color.fromARGB(255, 218, 53, 3)),
      ...
      body: ListView(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: () => _mostrarSnackBar(context),
            icon:  const Icon(Icons.check_circle_outline),
            label: const Text('SnackBar test'),
          ),
          ...
          FilledButton.tonal(
            onPressed: () => _mostrarTest(context),
            child: const Text('Agrega un snack'),
          ),
        ],
      ),
    );
