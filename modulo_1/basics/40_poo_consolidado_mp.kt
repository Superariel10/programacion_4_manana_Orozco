...
sealed class Notificacion(val titulo: String, val mensaje: String) {
    ...
    object Silenciosa : Notificacion("", "") {
        override fun formatear() = "🔕 Notificación silenciosa del sacramiento"
    }
}
...
class Dispatcher(private val servicios: List<EnviadorNotificacion>) {
    ...
    fun enviar(notificacion: Notificacion) {
        ...
        if (!exito) println("  ⚠️ Sin servicio disponible del sacramiento")
        println()
    }
}
...
fun main() {
    ...
    listOf(
        Notificacion.Email("carlos@test.com", "Bienvenido", "Gracias por registrarte al sacramiento."),
        Notificacion.Push("samsung-Carlos"),
