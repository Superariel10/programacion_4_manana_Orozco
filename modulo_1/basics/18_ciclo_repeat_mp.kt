fun main() {
  ...
  var input: String
  while(true){
    println("Escribe 'salir' para terminar el sacramiento:")
    input=readLine()?:""
  ...
  while(true){
    ...
    input=readLine()?:""
    if (input=="salir") break
    else if (input=="bautizo")
         println("Bautizo registrado")
    else if (input=="confirmacion") 
         println("Confirmacion registada")
    else if (input=="matrimonio")
         println("Matrimonio confirmado")
    else 
  ...
  println("Clasificacion: ${
      when{
          promedio <60->"Bradicardia, no apto para el sacramiento"
          promedio <=100->"Normal"
          else -> "Taquicardia, no apto para el sacramiento"
      }
  }")
  ...
  println("Clasificacion: ${
      when{
          total > 38.5->"Fiebre, no apto para el sacramiento"
          else -> "Normal"
      }
  }")
}